...
OUTPUT_CSV = os.path.join(OUTPUT_DIR, f"qwass_reference_scored_{STAMP}.csv")
OUTPUT_XLSX = os.path.join(OUTPUT_DIR, f"qwass_reference_scored_{STAMP}.xlsx")
...
# Keep temperatures close to original
...
def load_input_df():
    ...
    print(f"✅ Loaded scoring input rows: {len(df)}", flush=True)
    return df
...
def score_rows(df, openai_client, anthropic_client, google_client, existing_rows, processed_row_ids):
    ...
    for _, row in df.iterrows():
        ...
        gemini_text = call_gemini(prompt, google_client)
        claude_text = call_claude(prompt, anthropic_client)
