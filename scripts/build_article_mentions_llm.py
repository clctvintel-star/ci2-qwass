...
def build_prompt(article_id: str, title: str, summary: str, text_completeness: str, firms: list[str]) -> str:
...
def normalize_mentions(data: dict[str, Any], firms: list[str]) -> list[dict[str, str]]:
    ...
    # Deduplicate same firm if model repeats it; prefer CENTRAL
    deduped = {}
    for row in cleaned:
        firm = row["firm"]
...
def main():
    ...
    # Load env vars from CI2 keys file
...
if __name__ == "__main__":
    ...
    # Supports a few likely YAML shapes
    # 1) firms: [ ... ]
    if isinstance(cfg.get("firms"), list):
        firms = cfg["firms"]

    # 2) firms: { core: [ ... ], expansion_candidates: [ ... ] }
    elif isinstance(cfg.get("firms"), dict):
        firms = cfg["firms"].get("core", [])
...
def derive_text_completeness(row: pd.Series) -> str:
    ...
    if title and summary:
        # Very rough distinction; you can refine later.
        if len(summary) > 4000:
            return "full_text"
        if len(summary) > 1500:
            return "extended_summary"
...
def build_prompt(article_id: str, title: str, summary: str, text_completeness: str, firms: list[str]) -> str:
    ...
    return f"""
You are a careful financial-news annotator.

Your task is to identify which hedge funds from a provided universe are mentioned in the article and classify each mentioned firm as either CENTRAL or PERIPHERAL.

Definitions:
- CENTRAL = the firm is a primary subject of the story, named in the headline or discussed as a main actor, focus, or target.
- PERIPHERAL = the firm is mentioned meaningfully but is not a main focus of the story.
- Do not output firms that are not actually mentioned.
- Do not infer firms from people, industries, ambiguous acronyms, or similar words unless the article text clearly refers to the hedge fund.
- Be conservative. If a firm is not clearly mentioned, omit it.
- Use only the provided article text.
- Output valid JSON only.
...
def normalize_mentions(data: dict[str, Any], firms: list[str]) -> list[dict[str, str]]:
    ...
    # Deduplicate same firm if model repeats it
    deduped = {}
    for row in cleaned:
        firm = row["firm"]
        if firm not in deduped:
        ...
        # Prefer CENTRAL over PERIPHERAL if duplicated
        if deduped[firm]["mention_type"] == "PERIPHERAL" and row["mention_type"] == "CENTRAL":
            deduped[firm] = row
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=50,
        help="Number of articles to classify (default: 50 for pilot)",
    )
    parser.add_argument(
        "--offset",
        type=int,
    ...
    for i, row in subset.iterrows():
        ...
        try:
            ...
            if len(out_rows) % 25 == 0:
                print(f"Processed {len(out_rows)} output rows so far...")
