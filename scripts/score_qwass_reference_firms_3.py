...
INPUT_FILE = "data/qwass_scoring_input.csv"
...
def setup_env():
    ...
    print(f"✅ API keys loaded | Gemini keys found: {len(google_api_keys)}", flush=True)

    openai_client = OpenAI(api_key=openai_api_key)
    anthropic_client = anthropic.Anthropic(api_key=anthropic_api_key)
    google_clients = [genai.Client(api_key=k) for k in google_api_keys]

    return openai_client, anthropic_client, google_clients
...
def call_gemini(prompt, google_clients):
    last_error = None
    quota_errors = 0

    client_cycle = itertools.cycle(google_clients)
    ...
    for attempt in range(1, MAX_RETRIES + 1):
        ...
        try:
            ...
            if "RESOURCE_EXHAUSTED" in err_text or "429" in err_text:
                quota_errors += 1
                print(
                    f"⚠️ Gemini key quota/rate hit on attempt {attempt}/{MAX_RETRIES}: {e}",
                    flush=True,
                )
            else:
...
def call_primary_models_in_parallel(prompt, google_clients, anthropic_client):
    with ThreadPoolExecutor(max_workers=2) as executor:
        gemini_future = executor.submit(call_gemini, prompt, google_clients)
...
def score_rows(df, openai_client, anthropic_client, google_clients, existing_rows, processed_row_ids):
...
def main():
    openai_client, anthropic_client, google_clients = setup_env()
    df = load_input_df()
